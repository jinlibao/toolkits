...
data <- read.xls(file)
print(dim(data))
# std.data <- scale(data[, c(4, 6:10, 12)], center = TRUE, scale = TRUE)
...
lm.3 <- lm(Y1 ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X9, data = data)
print(summary(lm.3))
step.3 <- stepAIC(lm.3, direction = "both")
step.3$anova
print(summary(step.3))
sink()
